      ...
      if (valid){
        ...
        .then(({ status, json }) => {
          ...
          if (status === 201) {
            ...
            success.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m5 12 5 5L20 7"/></svg>';
            success.appendChild(document.createTextNode(' ' + (json.message || "Thanks — an advisor will reach out within one business day.")));
            form.reset();
            setTimeout(()=> success.classList.remove("show"), 6000);
          } else {
            success.className = "form-success show error";
            success.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>';
            success.appendChild(document.createTextNode(' ' + (json.error || "Submission failed. Please try again.")));
          }
        })
        .catch(err => {
          submitBtn.disabled = false;
          ...
          success.className = "form-success show error";
          success.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>';
          success.appendChild(document.createTextNode(' Connection error. Please check backend is running.'));
