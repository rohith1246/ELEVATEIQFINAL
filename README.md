...
You can log in to the dashboard (`http://localhost:5000/login.html`) using seeded test accounts.
**Passwords are auto-generated on first run** — check the server console output for credentials.
You can override defaults by setting environment variables: `ADMIN_PASSWORD`, `EMP_PASSWORD`, `STUDENT_PASSWORD`.

| Role | Email address / Login ID | Purpose |
| :--- | :--- | :--- |
| **Admin** | `admin@elevateiq.com` | Control panel, CRM lead generation, recruitment, notice boards. |
| **Employee** | `bathikadileep@gmail.com` | Check-in/out, apply leaves, workspace chat, view meetings. |
| **Candidate** | `rohith@gmail.com` | Job portal, application tracking status. |
