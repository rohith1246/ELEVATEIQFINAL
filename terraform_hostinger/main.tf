...
resource "null_resource" "hostinger_vps_deploy" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo bash -c 'cat <<EOT > /var/www/elevateiq/.env\nDATABASE_URL=postgresql://neondb_owner:npg_cS5VsRvqxl9H@ep-icy-dust-ainl946k-pooler.c-4.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require\nPORT=5000\nFLASK_ENV=production\nEOT'",
      "sudo cp /var/www/elevateiq/.env /var/www/assessments/.env",

      "cd /var/www/elevateiq && /var/www/elevateiq/venv/bin/python -c 'import bcrypt, psycopg2; conn=psycopg2.connect(\"postgresql://neondb_owner:npg_cS5VsRvqxl9H@ep-icy-dust-ainl946k-pooler.c-4.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require\"); cur=conn.cursor(); pw=bcrypt.hashpw(b\"Password123!\", bcrypt.gensalt()).decode(\"utf-8\"); cur.execute(\"UPDATE users SET password=%s, portal=\\\"elevateiq\\\" WHERE role IN (\\\"employee\\\", \\\"team_leader\\\", \\\"admin\\\", \\\"hr\\\", \\\"hr_manager\\\") OR email LIKE \\\"%%@gmail.com\\\";\", (pw,)); cur.execute(\"TRUNCATE TABLE account_lockouts, login_attempts;\"); conn.commit(); conn.close()' || true",
