...
resource "null_resource" "hostinger_vps_deploy" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo systemctl enable elevateiq elevateiq-assessment nginx",
      "sudo systemctl restart elevateiq elevateiq-assessment nginx",
      ...
      "echo '=== [DIAGNOSTICS] Listening Ports & Local Connectivity ==='",
      "sudo ss -tlpn || true",
