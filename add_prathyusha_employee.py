...
def add_prathyusha():
    ...
    name = "Prathyusha Kothapalli"
    email = "kothapalli.0706@gmail.com"
    phone = "9392267391"
    dept = "IT"
    ...
    try:
        ...
        next_num = (max_num + 1) if max_num else 122
        ...
        if row:
            user_id = row[0]
            c.execute(
                "UPDATE users SET name = %s, role = 'employee', portal = 'elevateiq' WHERE id = %s",
                (name, user_id)
            )
            print(f"Updated user ID {user_id}: {name} ({email})")
        else:
            c.execute(
        ...
        # Re-fetch to print full details
        c.execute("""
